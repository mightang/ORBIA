...
def draw_edge_hints(surface, board, center, size, font):
    ...
    for ent in board.edge_hints:
        ...
        # --- 기준 타일: JSON 지정(label_pos) 우선, 없으면 첫 내부 셀 ---
        anchor_qr = ent.get("label_pos")
        if not (isinstance(anchor_qr, (list, tuple)) and len(anchor_qr) == 2 and
                all(isinstance(v, (int, float)) for v in anchor_qr)):
            anchor_qr = first_inbounds_from(tuple(ent["pos"]), d)
        else:
            anchor_qr = (int(anchor_qr[0]), int(anchor_qr[1]))
        ax, ay = axial_to_pixel(anchor_qr[0], anchor_qr[1], size)
        ax += cx; ay += cy

        # --- 바깥 방향: JSON 지정(label_dir) 우선, 없으면 dir의 반대 ---
        offset_dir = ent.get("label_dir")
        offset_dir = ent.get("label_dir")
        if not isinstance(offset_dir, (int, float)):
            offset_dir = (d + 3) % 6
        offset_dir = int(offset_dir) % 6
        off_dx, off_dy = dir_pixel(offset_dir)
        off_norm = (off_dx*off_dx + off_dy*off_dy) ** 0.5 or 1.0

        # --- 거리: JSON 지정(label_dist) 우선, 없으면 EDGE_HINT_OFFSET ---
        ...
        # --- 회전: JSON 지정(label_angle) 있으면 그대로, 없으면 자동 ---
